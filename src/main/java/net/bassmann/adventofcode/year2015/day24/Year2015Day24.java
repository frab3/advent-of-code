package net.bassmann.adventofcode.year2015.day24;
 ...
 *
 * <p>It's Christmas Eve, and Santa is loading up the sleigh for this year's deliveries. However,
 ...
 * <p>No pressure.
 *
 * <p>Santa has provided you a list of the weights of every package he needs to fit on the sleigh.
 * The packages need to be split into three groups of exactly the same weight, and every package has
 * to fit. The first group goes in the passenger compartment of the sleigh, and the second and third
 * go in containers on either side. Only when all three groups weigh exactly the same amount will
 * the sleigh be able to fly. Defying physics has rules, you know!
 *
 * <p>Of course, that's not the only problem. The first group - the one going in the passenger
 ...
 *
 * <p>Furthermore, Santa tells you, if there are multiple ways to arrange the packages such that the
 * fewest possible are in the first group, you need to choose the way where the first group has the
 * smallest quantum entanglement to reduce the chance of any "complications". The quantum
 ...
 *
 * <p>For example, suppose you have ten packages with weights 1 through 5 and 7 through 11. For this
 ...
 * situation, the quantum entanglement for the ideal configuration is therefore 99. Had there been
 * two configurations with only two packages in the first group, the one with the smaller quantum
 ...
 */
public class Year2015Day24 extends AbstractDay {
