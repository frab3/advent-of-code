 ...
 * itself contains just one - an apostrophe ('), escaped using hexadecimal notation. Santa's list is
 * a file that contains many double-quoted string literals, one on each line. The only escape
 * sequences used are \\ (which represents a single backslash), \" (which represents a lone
 * double-quote character), and \x plus two hexadecimal characters (which represents a single
 ...
 * characters to 16. "\x27" encodes to "\"\\x27\"", an increase from 6 characters to 11. Your task
 * is to find the total number of characters to represent the newly encoded strings minus the number
 * of characters of code in each original string literal. For example, for the strings above, the
 * total encoded length (6 + 9 + 16 + 11 = 42) minus the characters in the original code
 * representation (23, just like in the first part of this puzzle) is 42 - 23 = 19.
 */
public class Year2015Day08 extends AbstractDay {

  public Year2015Day08() {
    super(2015, 8);
  }
  ...
  public String solvePart1() {
    final int diff =
        getRiddleInput().lines().mapToInt(Year2015Day08::getDecodedLengthDifference).sum();
  ...
  public String solvePart2() {
    final int diff =
        getRiddleInput().lines().mapToInt(Year2015Day08::getEncodedLengthDifference).sum();
    return Integer.toString(diff);
  }

  /**
   * Only counts how much characters less will be used after decoding.
   *
   * @param input to be decoded
   * @return input.length - decodedLength
   */
  static int getDecodedLengthDifference(String input) {
    final char[] charArray = input.toCharArray();
    int count = 2; // subtract surrounding quotes
    for (int i = 0, end = charArray.length; i < end; i++) {
      if (isEscapeCharacter(charArray[i])) {
        final int skip = (isHexIdentifier(charArray[i + 1])) ? 3 : 1;
        i += skip;
        count += skip;
      }
    }
  ...
  private static boolean isHexIdentifier(char c) {
    return c == 'x';
  }

  /**
   * Only counts how many more chars will be used after encoding.
   *
   * @param input to be encoded
   * @return encodedInput.length - input.length
   */
  static int getEncodedLengthDifference(String input) {
    int count = 2; // surrounded by extra quotes
  ...
  private static boolean isCharToEncode(char c) {
    return c == '\"' || c == '\\';
  }
}
